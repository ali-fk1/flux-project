...
public class LoginController {
    ...
    public Mono<ResponseEntity<Map<String, Object>>> login(@RequestBody LoginRequest request) {
        ...
        return authenticationService.authenticateUser(request.getEmail(), request.getPassword())
                .doOnSuccess(user -> log.info("✅ User authenticated: {}", user.getEmail()))
                .flatMap(authenticationService::generateAuthResponse)
                .map(response -> {
                    ...
                    // 🍪 CREATE COOKIES
                    String refreshCookie = cookieUtil.createRefreshTokenCookie(refreshToken);
                    String accessCookie  = cookieUtil.createAccessTokenCookie(accessToken);

                    log.info("✅ LOGIN SUCCESSFUL");
                    log.info("User: {}", request.getEmail());
                    log.info("Access token stored in HttpOnly cookie");
                    log.info("Refresh token stored in HttpOnly cookie");
                    log.info("Response body keys: {}", response.keySet());
                    log.info("============================================================");
    ...
    public Mono<ResponseEntity<Map<String, Object>>> refresh(
        ...
        return refreshTokenRepository.findByToken(refreshToken)
                ...
                .flatMap(token -> {
                    ...
                    return userRepository.findById(token.getUserId())
                            .flatMap(user ->
                                    tokenService.rotateRefreshToken(refreshToken, user.getId())
                                            .map(newRefreshToken -> {

                                                String newAccessToken =
                                                        tokenService.generateAccessToken(user.getId(), user.getEmail());
                                                ...
                                                String refreshCookie =
                                                        cookieUtil.createRefreshTokenCookie(newRefreshToken);
                                                String accessCookie =
                                                        cookieUtil.createAccessTokenCookie(newAccessToken);
    ...
    public Mono<ResponseEntity<Map<String, String>>> logout() {
        ...
        String clearRefresh = cookieUtil.clearRefreshTokenCookie();
        String clearAccess  = cookieUtil.clearAccessTokenCookie();
